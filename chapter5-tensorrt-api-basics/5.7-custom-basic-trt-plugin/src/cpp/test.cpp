...
bool build_model(){
    ...
    if(!parser->parseFromFile("models/onnx/sample_mySelu.onnx", 1)){
    ...
    FILE* f = fopen("models/engine/sample_mySelu.engine", "wb");
...
void inference(){
    ...
    TRTLogger logger;
    auto engine_data = load_file("models/engine/sample_mySelu.engine");
