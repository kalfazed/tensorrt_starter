...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.upsample = nn.Upsample(size=(6, 6), mode='nearest')
...
def export_weight(model):
    ...
    f = open(current_path + "/../../models/weights/sample_upsample.weights", 'w')
...
def export_norm_onnx(input, model):
    current_path = os.path.dirname(__file__)
    file = current_path + "/../../models/onnx/sample_upsample.onnx"
    torch.onnx.export(
        model         = model, 
        args          = (input,),
        f             = file,
