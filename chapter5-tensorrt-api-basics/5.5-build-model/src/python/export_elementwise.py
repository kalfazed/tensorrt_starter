...
class Model(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)
        # 创建一个常数张量并进行逐元素除法
        d = torch.tensor([255.0, 255.0, 255.0], dtype=torch.float32).view(1, 3, 1, 1)
        x = x / d
        return x
...
def export_weight(model):
    ...
    f = open(current_path + "/../../models/weights/sample_elementwise.weights", 'w')
...
def export_norm_onnx(input, model):
    current_path = os.path.dirname(__file__)
    file = current_path + "/../../models/onnx/sample_elementwise.onnx"
    torch.onnx.export(
        model         = model, 
        args          = (input,),
        f             = file,
