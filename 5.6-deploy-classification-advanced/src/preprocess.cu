    ...
    float* d_mean, float* d_std) 
{
    ...
    if (src_x < 0 || src_y < 0 || src_x > srcW || src_y > srcH) {
        ...
        tar[tarIdx + tarW * tarH * 0] = src[srcIdx + 2];
        tar[tarIdx + tarW * tarH * 1] = src[srcIdx + 1];
        tar[tarIdx + tarW * tarH * 2] = src[srcIdx + 0];

        if (tarIdx == 0) {
            printf("src idx = %d, tar idx = %d\n", srcIdx, tarIdx);
            printf("input value in (0, 0) with gpu is %f, %f, %f\n", 
                   tar[0], 
                   tar[1], 
                   tar[2]);
        }
    ...
    if (src_y1 < 0 || src_x1 < 0 || src_y2 > srcH || src_x2 > srcW) {
        ...
        if (tarIdx == 0) {
            printf("input value in (0, 0) with gpu is %f, %f, %f\n", 
                   ...
                   tar[tarArea * 1], 
                   tar[tarArea * 2]);
            // printf("src value in (0, 0)~(1, 1) with gpu is %d, %d, %d, %d\n", 
            //        src[srcIdx1_1 + 2],
            //        src[srcIdx1_2 + 2],
            //        src[srcIdx2_1 + 2],
            //        src[srcIdx2_2 + 2]);
    ...
    int tactis) 
{
    ...
    switch (tactis) {
    case 0:
        resize_bilinear_BGR2RGB_kernel <<<dimGrid, dimBlock>>> (d_tar, d_src, tarW, tarH, srcW, srcH, scaled_w, scaled_h, d_mean, d_std);
        // resize_nearest_BGR2RGB_kernel <<<dimGrid, dimBlock>>> (d_tar, d_src, tarW, tarH, srcW, srcH, scaled_w, scaled_h, d_mean, d_std);
        break;
    // case 1:
