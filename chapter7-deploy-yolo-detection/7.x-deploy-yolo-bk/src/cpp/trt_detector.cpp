...
void Detector::setup(nvinfer1::IRuntime& runtime, void const* data, size_t size) {
    /*
...
bool Detector::preprocess_cpu() {
    ...
    unsigned char* pimage = m_inputImage.data;
...
bool Detector::preprocess_gpu() {
    ...
    /*Preprocess -- 使用GPU进行双线性插值居中, 并将结果返回到m_inputMemory中*/
    process::preprocess_resize_gpu(m_inputImage, m_inputMemory[1],
                                   m_params->img.h, m_params->img.w, 
                                   process::tactics::GPU_BILINEAR_CENTER);
    return true;
}
...
bool Detector::postprocess_cpu() {
    ...
    for (int i = 0; i < bboxes_count; i ++) {
        ...
        // 这里的scale和shift是在preprocess中获取到的
        float scale    = 1.68750;
        float shift_x  = -134.656250;
        float shift_y  = 0.343750;
