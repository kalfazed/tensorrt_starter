...
def build_network(trtFile, shape_opt, shape, plugin):
    ...
    if os.path.isfile(trtFile):
        ...
        # 为network创建一个dummy的输入，并支持动态shape. 并且设置min, opt, max的shape profile
        inputT0 = network.add_input("inputT0", trt.float32, [-1 for i in shape])
        profile.set_shape(inputT0.name, shape_opt["min"], shape_opt["opt"], shape_opt["max"])
