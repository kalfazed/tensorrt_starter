#ifndef __CUSTOM_CUBLAS_SGEMM_PLUGIN_HPP__
#define __CUSTOM_CUBLAS_SGEMM_PLUGIN_HPP__
...
namespace custom 
{
static const char* PLUGIN_NAME {"customCublasSgemm"};
static const char* PLUGIN_VERSION {"1"};

class CustomCublasSgemmPlugin : public IPluginV2DynamicExt {
public:
    CustomCublasSgemmPlugin() = delete; 
    CustomCublasSgemmPlugin(const std::string &name, float* weight);
    CustomCublasSgemmPlugin(const std::string &name, const void* buffer, size_t length);

    ~CustomCublasSgemmPlugin();
...
private:
    ...
    int               mN;
    Weights           mTRTWeights;
    cublasHandle_t    mHandler;
};

class CustomCublasSgemmPluginCreator : public IPluginCreator {
public:
    CustomCublasSgemmPluginCreator();
    ~CustomCublasSgemmPluginCreator();
...
#endif //__CUSTOM_CUBLAS_SGEMM_PLUGIN_HPP__
