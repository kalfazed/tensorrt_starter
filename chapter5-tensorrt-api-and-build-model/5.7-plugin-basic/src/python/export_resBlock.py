...
def export_weight(model):
    ...
    f = open(current_path + "/../../models/weights/sample_resBlock.weights", 'w')
...
def export_norm_onnx(input, model):
    current_path = os.path.dirname(__file__)
    file = current_path + "/../../models/onnx/sample_resBlock.onnx"
    torch.onnx.export(
        model         = model, 
        args          = (input,),
        f             = file,
