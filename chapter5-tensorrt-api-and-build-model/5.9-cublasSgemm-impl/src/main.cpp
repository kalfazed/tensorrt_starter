...
int main(){
    ...
    /* cublas warmup */
    timer.start_gpu();
    cublasSgemmOnDevice(h_matN, h_matM, d_matP, width);
    timer.stop_gpu();
    std::sprintf(str, "matmul in gpu(with cublasSgemm(warmup))");
    timer.duration_gpu(str);

    /* cublas single precision gemm */
    timer.start_gpu();
    cublasSgemmOnDevice(h_matM, h_matN, d_matP, width);
    timer.stop_gpu();
    std::sprintf(str, "matmul in gpu(with cublasSgemm)");
    timer.duration_gpu(str);
    compareMat(h_matP, d_matP, size);
