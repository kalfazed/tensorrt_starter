...
class CustomScalarImpl(torch.autograd.Function):
    ...
    def symbolic(g, x, r, s):
        return g.op("custom::customScalarAttr", x, scalar_f=r, tag_s="scalar", scale_f=s) 
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.act    = CustomScalar(r=1, s=10)
...
def export_norm_onnx(input, model):
    ...
    file = current_path + "/../../models/onnx/sample_customScalarAttr.onnx"
    torch.onnx.export(
        model         = model, 
        args          = (input,),
        f             = file,
