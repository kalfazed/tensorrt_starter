...
public:
    ...
    */
    ...
    DataType    getOutputDataType(int32_t index, DataType const* inputTypes, int32_t nbInputs) const noexcept override; //获取op的输出数据类型，一般会和输入保持一致
    DimsExprs   getOutputDimensions(int32_t outputIndex, const DimsExprs* input, int32_t nbInputs, IExprBuilder &exprBuilder) noexcept override; //根据输入维度计算输出维度，支持动态batch
    size_t      getWorkspaceSize(const PluginTensorDesc *inputs, int32_t nbInputs, const PluginTensorDesc *outputs, int32_t nbOutputs) const noexcept override;

    /* */
    int32_t     initialize() noexcept override;   // 初始化Plugin, 开辟权重所需要的空间
