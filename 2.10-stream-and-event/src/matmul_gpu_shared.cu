...
void MatmulSharedOnDevice(float *M_host, float *N_host, float* P_host, int width, int blockSize, bool staticMem, cudaStream_t stream){
    ...
    CUDA_CHECK(cudaMemcpyAsync(M_device, M_host, size, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(N_device, N_host, size, cudaMemcpyHostToDevice, stream));
    ...
    if (staticMem) {
        MatmulSharedStaticKernel <<<dimGrid, dimBlock, 0, stream>>> (M_device, N_device, P_device, width);
    } else {
        MatmulSharedDynamicKernel <<<dimGrid, dimBlock, sMemSize, stream>>> (M_device, N_device, P_device, width, blockSize);
    }

    /* 将结果从device拷贝回host*/
    CUDA_CHECK(cudaMemcpyAsync(P_host, P_device, size, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaDeviceSynchronize());
